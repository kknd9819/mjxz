...
<title>${message("admin.article.edit")} -  生源闪购（shengyuan.cn）闪亮生活，购你喜欢！</title>
...
<body>
	<div class="path">
		<a href="/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.article.edit")}
	</div>
	<form id="inputForm" action="update.jhtml" method="post">
		...
		<table class="input">
			<tr>
				...
				<td>
					<input type="text" name="title" class="text" value="${article.title}" maxlength="20" />
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					<input type="hidden" id="parent" name="articleCategoryId" value="${article.articleCategory}" />
					<input type="text" id="parentName" class="text" value="${article.articleCategoryName}" onclick="showArticleCategoryTree();" readonly="true"/>
				</td>
			</tr>
			<tr>
				<th>
				...
				<td>
					[#list tags as tag]
						<label>
							...
							[#if articleTags?index_of(tag.id?string)>-1 ]
							checked="checked" 
							[/#if]
							/>${tag.name}
						</label>

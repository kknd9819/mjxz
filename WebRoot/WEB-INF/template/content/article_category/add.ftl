...
<title>${message("admin.articleCategory.add")} -  生源闪购（shengyuan.cn）闪亮生活，购你喜欢！</title>
...
<body>
	<div class="path">
		<a href="/admin/common/index.jhtml">${message("admin.path.index")}</a> &raquo; ${message("admin.articleCategory.add")}
	</div>
				...
				<td>
					<select name="position">
						[#list positions as position]
							<option value="${position.code}"[#if position == 0] selected="selected"[/#if]>${position.typeName}</option>
						[/#list]
					</select>
				</td>
			</tr>			
